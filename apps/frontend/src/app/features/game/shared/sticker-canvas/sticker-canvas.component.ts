...
export class StickerCanvasComponent implements AfterViewInit, OnDestroy {
    ...
    readonly selectionInfo = computed<{box: BoundingBox; rotation: number} | null>(() =>
        ops.computeSelectionInfo(this.stickers(), this.selectionIds(), id => this.getRenderedSize(id)),
    );
    ...
    undoAction(): void {
        ...
        if (prev) {this.clearSelection(); this.emitPlacements(prev);}
    }

    redoAction(): void {
        const next = this.undo.redo();
        if (next) {this.clearSelection(); this.emitPlacements(next);}
    }
    ...
    private emitPlacements(updated: StickerPlacement[]): void {this.placementsChanged.emit(updated);}
